...
FB.api = function (par1, par2, par3, par4) {
    ...
    var tmp = par1.indexOf('?');
    if (tmp != -1) {
        if (par1.length > tmp + 1) {
            var temParams = '({' + par1.substring(tmp + 1) + '})';
            temParams = temParams.replace(/&/g, ',');
            temParams = temParams.replace(/=/g, ':');
            temParams = eval(temParams);
            if (params) {
                var prop;
                for (prop in temParams)
                    params[prop] = temParams[prop];
            }
            else
                params = temParams;
        }
        par1 = par1.substring(0, tmp);
