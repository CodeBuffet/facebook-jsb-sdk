...
bool JSB_Facebook_login(JSContext *cx, uint32_t argc, jsval *vp){
    int cbIndex;
    JSString * str = NULL;
    ...
    if (str)
    {
        ...
        FacebookInterface::login(cbIndex,wrapper.get());
    }
    else
...
bool JSB_Facebook_logout(JSContext *cx, uint32_t argc, jsval *vp){
    int cbIndex;
...
bool JSB_Facebook_getLoginStatus(JSContext *cx, uint32_t argc, jsval *vp){
...
bool JSB_Facebook_api(JSContext *cx, uint32_t argc, jsval *vp){
...
bool JSB_Facebook_ui(JSContext *cx, uint32_t argc, jsval *vp){
...
void register_facebook_js(JSContext* cx, JSObject* global){
    ...
    JS::RootedObject facebookJsbObject(cx);
